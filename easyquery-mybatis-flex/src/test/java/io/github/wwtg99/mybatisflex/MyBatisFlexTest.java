...
public class MyBatisFlexTest {
    ...
    public void testQueryWrapper() {
        ...
        Assertions.assertEquals("SELECT * FROM `table` WHERE name = 'name' AND age > 30", queryWrapper.toSQL());
        obj1 = Obj1.builder().name("name").ageGt(35).ageLt(40).nameNe("no").address("address").query("query").build();
        queryWrapper = wrapper.build(obj1);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE name = 'name' AND age > 35 AND age < 40 AND name != 'no' AND ((address LIKE 'query%') OR (name LIKE 'query%'))", queryWrapper.toSQL());
        obj1 = Obj1.builder().sizeGe(100).sizeLe(200).types(List.of("t1", "t2")).noTypes(List.of("t3")).isNull(true).build();
        queryWrapper = wrapper.build(obj1);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE size >= 100 AND size <= 200 AND type IN ('t1', 't2') AND type NOT IN ('t3') AND flag IS NULL", queryWrapper.toSQL().trim());
        obj1 = Obj1.builder().sizeGe(100).types(Collections.emptyList()).isNotNull(true).build();
        queryWrapper = wrapper.build(obj1);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE size >= 100 AND flag IS NOT NULL", queryWrapper.toSQL().trim());
        obj1 = Obj1.builder().types(List.of("t1")).name("name").isNull(false).build();
        queryWrapper = wrapper.build(obj1);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE name = 'name' AND type IN ('t1')", queryWrapper.toSQL().trim());
        ...
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE (name LIKE 'left%') AND (name LIKE '%right') AND (name LIKE '%both%') ORDER BY sort DESC, date DESC", queryWrapper.toSQL());
        obj2 = Obj2.builder().bothLike("both").sort(true).dateDesc(true).build();
        queryWrapper = wrapper.build(obj2);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE name LIKE '%both%' ORDER BY sort ASC, date DESC", queryWrapper.toSQL());
        obj2 = Obj2.builder().bothLike("both").sort(null).dateDesc(false).build();
        queryWrapper = wrapper.build(obj2);
        queryWrapper.from(TABLE_NAME);
        Assertions.assertEquals("SELECT * FROM `table` WHERE name LIKE '%both%' ORDER BY date ASC", queryWrapper.toSQL());
    }
}
